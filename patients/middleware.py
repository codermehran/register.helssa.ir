...
class PublicVisitTrackingMiddleware:
    ...
    def __call__(self, request):
        ...
        if self._should_track(request):
            form_paths = tuple(getattr(settings, "ANALYTICS_FORM_GET_PATHS", self.FORM_GET_PATHS))
            event_type = VisitEvent.EVENT_FORM_VIEW if request.path in form_paths else VisitEvent.EVENT_PAGE_VIEW
    ...
    def _should_track(self, request):
        ...
        if any(request.path.startswith(prefix) for prefix in self.SKIP_PREFIXES):
            return False
        tracked_paths = tuple(getattr(settings, "ANALYTICS_TRACKED_GET_PATHS", self.TRACKED_GET_PATHS))
